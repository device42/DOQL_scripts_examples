/*   Trend Report - OS
          Created 2/19/2021
           Report 2 - Over the past year show the OS group for the top 5 groups
      */
      With
       /* Get all the OSes for devices cloud and on prem  */
      dos_init  as
        (select
            d.device_pk
            ,ci.device_fk
      ...
      dos  as
        (select
			di.*
			,Case When os_group IN ('Partial Discovery', 'Other') Then os_group
				When os_group = 'Windows' Then di.os_name 
				Else concat (di.os_name,'-',split_part(coalesce(di.os_version,di.os_version_no,''),'.',1))
			End os_name_cat
          From
            dos_init di 
		),
	/* Rank the OSs based upon volume  */	
      dos_ranks as
        (select os_name_cat
              ,total_devices
      ...
      dos_all as
        (select dos.*
              ...
              /* This is where we set the number of OSs that will not be lumped together */
              ,case when dos_ranks.rank_os_count <= 10 then dos.os_name_cat
                  when dos.os_name_cat like 'Other' then 'Other'
                  else 'Other'
                  end as os_group_w_other
          from dos
          left join dos_ranks
            on dos.os_name_cat = dos_ranks.os_name_cat),
      month_date_table as
