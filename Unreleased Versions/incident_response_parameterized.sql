  ...
  pdu_biz_apps AS (
    WITH 
          ...
          impacted_devices AS (
            ...
            LEFT JOIN view_businessapplication_v1 b on b.businessapplication_pk = bae.businessapplication_fk
            LEFT JOIN view_ipaddress_v1 ip on ip.device_fk = d.device_pk
            LEFT JOIN view_ipaddress_v1 hip on hip.device_fk = h.device_pk               
          )
    SELECT SPLIT_PART(p.name,' ',1) "object_name"
           ...
           ,id.device_ip "impacted_ip_address"
           ,CASE WHEN id.host IS NOT NULL THEN id.host 
                 ELSE 'No Host'
           END "device_host"
           ,CASE WHEN id.host IS NOT NULL THEN id.host_ip::text
                 ELSE 'No Host'
           END "host_ip_address"
           ,id.device_subtype
    ...
    WHERE LOWER(p.name) LIKE LOWER(CONCAT('%', {search_parameter},'%')) OR LOWER(id.device) LIKE LOWER(CONCAT('%', {search_parameter},'%')) OR LOWER(id.host) LIKE LOWER(CONCAT('%', {search_parameter},'%'))
          OR LOWER(id.device_ip::text) = {search_parameter} OR LOWER(id.host_ip::text) = {search_parameter}
  ),
...
/* Get virtual disks on storage arrays and their relationships to business app infrastructure*/
  vdisk_storage_biz_apps AS (
    WITH
        ...
        LEFT JOIN view_businessapplicationelement_v1 bae on bae.device_fk = vd.virtualmachine_fk
        LEFT JOIN view_businessapplication_v1 ba on ba.businessapplication_pk = bae.businessapplication_fk
        JOIN view_device_v2 d on vd.virtualmachine_fk = d.device_pk
        LEFT JOIN view_ipaddress_v1 ip on ip.device_fk = d.device_pk
        LEFT JOIN view_ipaddress_v1 hip on hip.device_fk = vd.hypervisor_fk
    ...
    SELECT p.storagearray_name "object_name"
           ...
           ,p.device_ip "impacted_ip_address"
           ,p.host "device_host"
           ,p.host_ip::text "host_ip_address"
           ,p.device_subtype
    ...
    FROM vdisks p
    WHERE LOWER(p.storagearray_name) LIKE LOWER(CONCAT('%', {search_parameter},'%')) OR LOWER(p.vm_name) LIKE LOWER(CONCAT('%', {search_parameter},'%')) OR LOWER(p.host) LIKE LOWER(CONCAT('%', {search_parameter},'%'))
          OR LOWER(p.device_ip::text) = {search_parameter} OR LOWER(p.host_ip::text) = {search_parameter}
  ),
...
SELECT DISTINCT 
       ...
       ,p."business_app_id"
       ,p."relationship_type"
FROM related_object_union p
LEFT JOIN biz_apps ba on p."business_app_id" = ba.businessapplication_pk
/* WHERE LOWER(p."object_name") LIKE '%juniper-wh-01%' OR LOWER(p."impacted_object") LIKE '%juniper-wh-01%' OR LOWER(p."device_host") LIKE '%juniper-wh-01%' */
