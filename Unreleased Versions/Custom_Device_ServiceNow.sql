...
select 
       d.name as "CI Name",
       CASE When lower(d.type) IN ('virtual')
			Then 'True'
			Else 'False'
		End	"Is Virtual",
       substring(d.name from '\.(.*)$') as "DNS Domain",
       case when lower(d.os_name) IN ('windows', 'microsoft') then 'Windows Server'
            when lower(d.os_name) IN ('amazon', 'ubuntu', 'centos', 'linux', 'redhat', 'debian', 'sles', 'redhatenterpiseserver', 'suse', 'gentoo', 'oracle', 'freebsd', 'rhel', 'red hat enterprise linux server', 'redhatenterprisees', 'fedora') then 'Linux Server'
            when lower(d.os_name) IN ('aix', 'hmc') then 'AIX Server'
            when lower(d.os_name) IN ('solaris', 'sunos') then 'Solaris Server'
            when lower(d.os_name) IN ('f5', 'netscaler') then 'Load Balancer'
            when lower(d.os_name) IN ('esxi') then 'ESX Server'
            when lower(d.os_name) IN ('ios', 'cisco', 'ftos', 'junos', 'phone load id', 'unified communications manager', 'fortios', 'acos', 'comware', 'ios-xe', 'ironware', 'huawei', 'asa', 'linksys', 'pco', 'pan-os', 'airos', 'alteon', 'qts', 'dsm', 'asyncos', 'exos', 'onos') then 'IP Switch'
            Else 'No Subcategory'
        END "Subcategory",
       'Device42' as "Discovery Source",
       CASE When d.in_service = 't'
			Then 'Deployed'
			Else 'Not Deployed'
		End	"Status(install_status)",
       coalesce(d.physicalsubtype, '') || coalesce(d.virtualsubtype, '') as "Category",
       d.device_pk as "Discovery Key",
       null as "Component Aliases",
       d.serial_no as "Serial Number",
       ps.cpu_manufacturer as "CPU manufacturer",
       ps.cpu_model as "CPU Name",
       ps.cpu_string,
       d.total_cpus as "CPU Count",
       d.cpu_speed as "CPU speed (MHz)",
       d.core_per_cpu as "CPU Core Count",
       d.threads_per_core as "CPU Core Thread",
       'TRUE' as "Asset",
       case when d.ram <= 0 or d.ram is null then null                        
             ...
             else null end as "RAM (MB)",
       d.name as "Host name",
       dcf."Domain" as "Domain",
       ...
       ds.disk_type,
       string_agg(distinct np.hwaddress::text, ' | ') as "MAC Address(s)",
...
left join view_netport_v1 np on np.device_fk =  d.device_pk
